...
import com.spring.tableau.bindings.ViewType;
...
public class GetViewList {
	...
	public  List<MultiLaunchWbEntity> getPjList(){
		...
		List<ViewType> currentUserViews = s_restApiUtils
				.invokeQueryViews(credential, currentSiteId, currentUserId)
				.getView();
		int i;
		List<MultiLaunchWbEntity> wbList = new ArrayList<MultiLaunchWbEntity>();
		
		for (i = 0; i < currentUserViews.size() - 1; i++) {
			ViewType ViewTypeObj = currentUserViews.get(i);
			MultiLaunchWbEntity wbObj = new MultiLaunchWbEntity();
			wbObj.setViewName(ViewTypeObj.getName());
			wbList.add(wbObj);
		}
