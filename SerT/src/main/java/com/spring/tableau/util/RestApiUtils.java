package com.spring.tableau.util;
...
import com.spring.tableau.bindings.CapabilityType;
import com.spring.tableau.bindings.FileUploadType;
import com.spring.tableau.bindings.GranteeCapabilitiesType;
import com.spring.tableau.bindings.GroupType;
import com.spring.tableau.bindings.ObjectFactory;
import com.spring.tableau.bindings.PermissionsType;
import com.spring.tableau.bindings.ProjectListType;
import com.spring.tableau.bindings.ProjectType;
import com.spring.tableau.bindings.SiteListType;
import com.spring.tableau.bindings.SiteType;
import com.spring.tableau.bindings.TableauCredentialsType;
import com.spring.tableau.bindings.TsRequest;
import com.spring.tableau.bindings.TsResponse;
import com.spring.tableau.bindings.ViewListType;
import com.spring.tableau.bindings.WorkbookListType;
...
public class RestApiUtils {
	...
	private enum Operation {
		...
		QUERY_VIEWS(getApiUriBuilder().path("sites/{siteId}/users/{usertId}/views")),
		SIGN_IN(getApiUriBuilder().path("auth/signin")), SIGN_OUT(getApiUriBuilder().path("auth/signout"));
	...
	/**
	 * Initializes the RestApiUtils. The initialize code loads values from the
	 * configuration file and initializes the JAXB marshaller and unmarshaller.
	 */
	private static void initialize() {
		try {
			ClassLoader currentThreadClassLoader = Thread.currentThread().getContextClassLoader();

			URL url_config = currentThreadClassLoader.getResource("config.properties"); 
			m_properties.load(url_config.openStream());
			URL url_schema = currentThreadClassLoader.getResource(m_properties.getProperty("server.schema.location")); 
...
//			m_properties.load(new FileInputStream("res/config.properties"));
			...
			Schema schema = schemaFactory.newSchema(new File(url_schema.getFile()));
	...
	public PermissionsType invokeAddPermissionsToWorkbook(TableauCredentialsType credential, String siteId,
		...
		System.out.println("invokeAddPermissionToWorkbook 吏꾩엯");
	 ...
	 * @param requestPayload the payload used to create the group					占쎈읂占쎌뵠嚥≪뮆諭뜻에�뮇苑� 域밸챶竊숋옙�뱽 筌띾슢諭얏묾占� 占쎌맄占쎈퉸
	...
	public ViewListType invokeQueryViews(TableauCredentialsType credential, String siteId, String userId) {

		m_logger.info(String.format("Querying views on site '%s'.", siteId));

		String url = Operation.QUERY_VIEWS.getUrl(siteId, userId);
	...
	private void invokeAppendFileUpload(TableauCredentialsType credential, String siteId, String uploadSessionId,
		...
		// Writes the the chunk of data to a temporary file
